...
const nextConfig = {
  ...
  async rewrites() {
    return [
      ...
      {
        source: `/api/v1/citys`,
        destination: `${process.env.API_URL}/api/v1/citys/`,
      },
      // Запрос категории
      {
        source: `/api/v1/category`,
        destination: `${process.env.API_URL}/api/v1/category/`,
      },
      //Список всех популярных продуктов
      {
        source: `/api/v1/populates`,
        destination: `${process.env.API_URL}/api/v1/populates/`,
      },
      // Список всех продуктов
      {
        source: `/api/v1/products`,
        destination: `${process.env.API_URL}/api/v1/products/`,
      },
      // Детали конкретного продукта по его слагу
      {
        source: `/api/v1/products/:slug_prod`,
        destination: `${process.env.API_URL}/api/v1/products/:slug_prod/`,
      },
      // Фильтрация продуктов по категории
      {
        source: `/api/v1/products/filter_by_cat/:slug_cat`,
        destination:
          `${process.env.API_URL}/api/v1/products/filter_by_cat/:slug_cat/`,
      },
      // Получение списка слагов всех продуктов
      {
        source: `/api/v1/products/all/slugs`,
        destination: `${process.env.API_URL}/api/v1/products/all/slugs/`,
      },
      //Получение продуктов по списку идентификаторов
      {
        source: `/api/v1/products/by_ids/:ids`,
        destination: `${process.env.API_URL}/api/v1/products/by_ids/:ids/`,
      },
      //Фильтрация продуктов по различным параметрам
      {
        source: `/api/v1/products/set/filter`,
        destination: `${process.env.API_URL}/api/v1/products/set/filter/`,
      },
      // ревью (обзоры)
      {
        source: `/api/v1/reviews/filter_by_prod/:prod_pk`,
        destination:
          `${process.env.API_URL}/api/v1/reviews/filter_by_prod/:prod_pk/`,
      },
      //Спецификации на товар
      {
        source: `/api/v1/specif/filter_by_prod/:prod_pk`,
        destination: `${process.env.API_URL}/api/v1/specif/filter_by_prod/:prod_pk/`,
      },

      // Обработка картинок (проксирование)
      {
        source: `/media/product_images/:patch*`,
        destination: `${process.env.API_URL}:8000/media/product_images/:patch*/`,
      },
      ...
      // Запрос данных их корзины
      {
        ...
        destination: `${process.env.API_URL}:8777/basket_api/v1/bascket/:url*/`,
      },

      // api заказа
      {
        source: `/basket_api/v1/order/:url*`,
        destination: `${process.env.API_URL}:8777/basket_api/v1/order/:url*/`,
      },
      ...
      // Запрос данных пользователя (инфо)
      {
        ...
        destination: `${process.env.API_URL}:8999/auth_api/v1/user/info`,
      },
      {
        source: `/auth_api/v1/token/refresh`,
        destination: `${process.env.API_URL}:8999/auth_api/v1/token/refresh`,
      },
      ...
      //Получение ссылки на авторизацию google
      {
        ...
        destination:
          `${process.env.API_URL}:8999/auth_api/v1/auth_user/login/google/`,
      },
      //api по работе с пользователем
      // Создаем пользователя через акаунт google
      {
        source: `/auth_api/v1/auth_user/auth/google`,
        destination: `${process.env.API_URL}:8999/auth_api/v1/auth_user/auth/google`,
      },
      ...
      //Endpoint для отправки sms-кода
      {
        ...
        destination: `${process.env.API_URL}:8999/auth_api/v1/auth_user/login/phone`,
      },
      //api по работе с пользователем
      //Через данный endpoint будут выданы ключи доступа.
      {
        source: `/auth_api/v1/auth_user/auth/phone`,
        destination: `${process.env.API_URL}:8999/auth_api/v1/auth_user/auth/phone`,
      },
      //api по работе с пользователем
